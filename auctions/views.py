...
class CreateListingForm(forms.Form):
    ...
    description = forms.CharField(label="Description (max. 1024 characters)", widget=forms.Textarea(
        attrs={'cols': 10, 'rows': 8}), max_length=1024)
    category = forms.CharField(label="Category", max_length=32)
    photo_url = forms.URLField(label="Photo URL (optional)", required=False)
    starting_bid = forms.DecimalField(label="Starting bid (USD)", decimal_places=2,
                                      max_digits=7, max_value=9999.99, widget=forms.TextInput(attrs={'id': 'bid-field'}))
...
class CommentForm(forms.Form):
    comment = forms.CharField(label="Leave comment", widget=forms.Textarea(
        attrs={'cols': 10, 'rows': 3}), max_length=1024)
...
def add_bidinfo(listings):
    for listing in listings:
        if Bid.objects.filter(listing=listing.id):
            ...
            listing.highest_bid = Bid.objects.filter(
                listing=listing.id).order_by('-bid_amount')[0].bid_amount
            listing.highest_bidder = Bid.objects.filter(
                listing=listing.id).order_by('-bid_amount')[0].user
        else:
...
def listing(request, listing_id):
    ...
    if Bid.objects.filter(listing=listing_id):
        current_highest_bid = Bid.objects.filter(
            listing=listing_id).order_by('-bid_amount')[0].bid_amount
        current_highest_bidder = Bid.objects.filter(
            listing=listing_id).order_by('-bid_amount')[0].user
    else:
    ...
    # or_create returns a tuple and so requires the 'created' part which returns True if created and False if not
    try:
    ...
    class BidForm(forms.Form):
        bid_amount = forms.DecimalField(label="Bid amount", decimal_places=2, max_digits=7, min_value=min_bid,
                                        max_value=10000, widget=forms.NumberInput(attrs={'placeholder': f"{min_bid:.2f} or more", 'id': 'bid-field'}))
    ...
    if request.method == "POST":
        ...
        if "bid_amount" in request.POST:
            ...
            if bid_form.is_valid():
                ...
                bid_form = BidForm(
                    initial={"bid_amount": request.POST["bid_amount"]})
...
def category(request, category_name):
    ...
    listings = Listing.objects.filter(
        is_active=True).filter(category=category_name)
    add_bidinfo(listings)
...
def mybids(request):
    ...
    listings = Listing.objects.filter(
        bids__user=request.user).order_by("-is_active")
    bid_listings = []
